...
class TestNewComplexCoverageInt(CoverageModelIntTestCase, CoverageIntTestBase):
    ...
    def test_file_mode(self):
        ...
        while rpt > 0:
            ...
            np.testing.assert_array_equal(time_value.get_data()['time'], np.array([0,1,2]*3, dtype=np.float32))
            read_ccov.close()
            del(read_ccov)
            rpt = rpt - 1
    ...
    def test_get_some_parameters(self):
        ...
        times = np.empty(size*2, dtype='float32')
        times[0:size] = first_times
        times[size:size*2] = second_times
        fill_data = np.empty(size*2, dtype='float32')
        fill_data.fill(-9999.0)
        adata = fill_data.copy()
        adata[0:size] = first_data
